...
def checkForPython2():
  try:
    ...
    print("transitfeed requires python2, stupid thing, which you don't have. gotta go get that. exiting")
    sys.exit(1)
...
def downloadAndCheckFeed_TransitFeeds_FeedId(feedId):
  ...
  if len(versions) == 0:
    logger.info(f'No feeds at all for {feedId}')
    return False
...
def downloadAndCheckFeed_TransitFeeds(feed):
  if 'location' in feed:
    ...
    print(f"don't know how to parse transitfeeds feed {feed}")
    sys.exit(1)
...
def runOTP():
  ...
  subprocess.run(["java", "-Xmx10G", "-jar", otpJarName, '--build', OTP_INPUT_DIR])
  GRAPH_FILE = 'Graph.obj'
  graphFilePath = os.path.join(OTP_INPUT_DIR, GRAPH_FILE)
  if os.path.exists(graphFilePath):
    print(f'built index at {graphFilePath}')

    # copy graph from OTP_INPUT_DIR/Graph.obj to OTP_OUTPUT_DIR/date_str/Graph.obj
    dateStr = datetime.datetime.now().strftime('%Y-%m-%d-%H-%M')
    outputDir = os.path.join(OTP_OUTPUT_DIR, dateStr)
    outputPath = os.path.join(outputDir, GRAPH_FILE)
    os.mkdir(outputDir)
    shutil.copy(graphFilePath, outputPath)

    # symlink OTP_OUTPUT_DIR/datestr/ to OTP_OUTPUT_DIR/default/
    currentLinkPath = os.path.join(OTP_OUTPUT_DIR, DEFAULT_GRAPH_NAME)
    if os.path.islink(currentLinkPath):
      os.unlink(currentLinkPath)
    os.symlink(dateStr, currentLinkPath)

    print(f'All done at {outputPath}')
